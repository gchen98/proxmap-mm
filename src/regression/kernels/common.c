...
__local float * local_floatgeno
  ...
  if (subject_index<observations){
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    local_floatgeno[threadindex]= local_floatgeno[threadindex]==9?0:(local_floatgeno[threadindex]-mean)*precision*lambda[subject_index];
    barrier(CLK_LOCAL_MEM_FENCE);
    // REDUCE 
    for(unsigned int s=BLOCK_WIDTH/2; s>0; s>>=1) {
      if (threadindex < s) {
    ...
    if(threadindex==0){
      Xt_lambda_chunks[snp*subject_chunks+subject_chunk] = local_floatgeno[0];
    }
  }
  return;
}

__kernel void reduce_xt_lambda_chunks(
const unsigned int observations,
const unsigned int subject_chunks,
const unsigned int chunk_clusters,
__global float * Xt_lambda_chunks,
__global float * Xt_lambda,
__local float * local_xt
  ...
  for(int chunk_cluster=0;chunk_cluster<chunk_clusters;++chunk_cluster){
    ...
    if(subject_chunk<subject_chunks){
      local_xt[threadindex] += Xt_lambda_chunks[snp*subject_chunks+subject_chunk];
      barrier(CLK_LOCAL_MEM_FENCE);
    }
  }
  // REDUCE 
  ...
  if(threadindex==0){
    Xt_lambda[snp] = local_xt[0];
  }
}

__kernel void compute_xxi_lambda(
const unsigned int observations,
const unsigned int subject_chunks,
__global float * XXI,
__global float * lambda,
__global float * XXI_lambda,
__local float * local_xxi_lambda
){
  int sub_observation = get_group_id(1);
  int threadindex = get_local_id(0);
  local_xxi_lambda[threadindex] = 0;
  barrier(CLK_LOCAL_MEM_FENCE);
  for(int subject_chunk=0;subject_chunk<subject_chunks;++subject_chunk){
    int observation = subject_chunk*BLOCK_WIDTH+threadindex;
    if(observation<observations){
      //local_xxi_lambda[threadindex]+=1;
      local_xxi_lambda[threadindex]+=XXI[sub_observation*observations+observation] * lambda[observation];
      barrier(CLK_LOCAL_MEM_FENCE);
    }
  }
  for(unsigned int s=BLOCK_WIDTH/2; s>0; s>>=1) {
    if (threadindex < s) {
      local_xxi_lambda[threadindex] += local_xxi_lambda[threadindex + s];
    }
    barrier(CLK_LOCAL_MEM_FENCE);
  }
  if(threadindex==0){
    XXI_lambda[sub_observation] = local_xxi_lambda[0];
  }
}
